...
int print_help() {
    ...
    printf("  -t MQTT_TOPIC (w1toha): MQTT root topic\n");
...
int main(int argc, char * const argv[]) {
    ...
    while ((opt = getopt(argc, argv, "srhPtupia")) != -1) {
        if (optind > argc) return print_help();
        switch (opt) {
            case 'r':
                if (portname == NULL) return print_help();
                if (max_device == 0) devices = malloc(sizeof(struct device_t));
                else devices = realloc(devices, (max_device + 1) * sizeof(struct device_t));
    ...
    if (!max_device || !mqtt_host || !mqtt_user || !mqtt_pass) return print_help();
    ...
    // set up a client ID
    char *client_id = alloc_client_id();
    printf("mqtt client ID is %s\n", client_id);

    // make a will
    char mqtt_topic[MQTT_MAX_PACKET_SIZE];
    char mqtt_message[1024];
    snprintf(mqtt_topic, sizeof(mqtt_topic), "%s/%s", mqtt_root, client_id);
    snprintf(mqtt_message, sizeof(mqtt_message), "{\"av\":\"offline\"}");

    printf("authenticating as %s...\n", mqtt_user);
    if (send_connect(sockfd, client_id, mqtt_user, mqtt_pass, (uint16_t) interval * 3 + 10, mqtt_topic, mqtt_message) < 0) {
        close(sockfd);
    ...
    while (1) {
        ...
        // update availability
        snprintf(mqtt_topic, sizeof(mqtt_topic), "%s/%s", mqtt_root, client_id);
        snprintf(mqtt_message, sizeof(mqtt_message), "{\"av\":\"online\"}");
        if (send_publish(sockfd, mqtt_topic, mqtt_message, 0, 0) < 0) {
            close(sockfd);
                ...
                if (read_ds18b20(&devices[i]) < 0) {
                    ...
                    if (announce_every >= 0 && !counter) {
                        ...
                        snprintf(
                            mqtt_message,
                            sizeof(mqtt_message),
                            "{\"name\":\"Temperature\",\"stat_t\":\"%s/%016lx\",\"uniq_id\":\"%s-%016lx_T\",\"obj_id\":\"%s-%016lx_T\",\"unit_of_meas\":\"\\u00b0C\",\"val_tpl\":\"{{ value_json.T | is_defined }}\",\"dev\":{\"ids\":[\"%s-%016lx\"],\"name\":\"DS18B20 sensor: %016lx\"},\"dev_cla\":\"temperature\",\"avty_t\":\"%s/%s\",\"avty_tpl\":\"{{ value_json.av | is_defined }}\"}",
                        ...
                        if (send_publish(sockfd, mqtt_topic, mqtt_message, MQTT_FLAG_PUBLISH_RETAIN, 0) < 0) {
                            close(sockfd);
                            fprintf(stderr, "failed to publish to MQTT!\n");
                            return 1;
                        }
